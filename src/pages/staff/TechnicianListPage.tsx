...
const STATUS_BG: Record<string, string> = { free: 'var(--ok-soft)', busy: 'var(--brand-soft)', off: 'var(--surface-3)' };
const STATUS_TEXT: Record<string, string> = { free: '#1B6F4D', busy: 'var(--brand-2)', off: 'var(--ink-4)' };
...
const TechnicianListPage: React.FC = () => {
  ...
  const addBtn = (
    <button onClick={() => setShowModal(true)}
      ...
      style={{ height: 36, fontSize: 13, padding: '0 14px', display: 'flex', alignItems: 'center', gap: 6 }}>
      <Plus size={15} stroke="#fff" />
      เพิ่มช่าง
    </button>
  );
    ...
    <PhoneShell title="ช่างซ่อม" rightAction={addBtn}>
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '64px 16px', color: 'var(--ink-4)' }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>🔧</div>
            <div style={{ fontSize: 15, fontWeight: 500 }}>ยังไม่มีช่างซ่อม</div>
            <div style={{ fontSize: 13, marginTop: 4 }}>กดปุ่ม "เพิ่มช่าง" เพื่อเริ่มต้น</div>
          </div>
        ) : (
          <div style={{
            display: 'grid',
            ...
            {technicians.map(tech => {
              ...
              const initial = tech.name.trim().slice(-2);
              return (
                <div key={tech.id} className="dn-card" style={{ padding: '16px', transition: 'box-shadow .15s' }}
                  ...
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    ...
                    <div style={{
                      ...
                      background: catColor + '20', display: 'flex', alignItems: 'center',
                      justifyContent: 'center', fontSize: 14, fontWeight: 700, color: catColor,
                    }}>
                      {initial}
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink-1)', marginBottom: 3 }}>
                        {tech.name}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        <CatIcon size={11} stroke={catColor} />
                        <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>
                          {tech.role || CATEGORY_LABEL[tech.team]}
                        </span>
                      </div>
                    </div>
                    ...
                    <div style={{
                      ...
                      <div style={{ width: 6, height: 6, borderRadius: '50%', background: STATUS_DOT[tech.status] }} />
                      <span style={{ fontSize: 11, fontWeight: 500, color: STATUS_TEXT[tech.status] }}>
                  ...
                  {(tech.rating > 0 || tech.completed_jobs > 0) && (
                    <div style={{
                      ...
                      {tech.rating > 0 && (
                        <span style={{ fontSize: 12, color: 'var(--ink-4)' }}>⭐ {tech.rating.toFixed(1)}</span>
                      )}
                      {tech.completed_jobs > 0 && (
                        <span style={{ fontSize: 12, color: 'var(--ink-4)' }}>✅ {tech.completed_jobs} งาน</span>
                      )}
                      {tech.avg_time_hours > 0 && (
                        <span style={{ fontSize: 12, color: 'var(--ink-4)' }}>⏱ {tech.avg_time_hours} ชม.</span>
                      )}
                    </div>
                  )}

                  {/* Phone button */}
                  {tech.phone && tech.status !== 'off' && (
                    <a href={`tel:${tech.phone}`} style={{
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                      marginTop: 10, padding: '7px 0', borderRadius: 'var(--r-md)',
                      background: 'var(--brand-soft)', color: 'var(--brand-2)',
                      fontSize: 13, fontWeight: 500, textDecoration: 'none',
                    }}>
                      <Phone size={13} stroke="var(--brand-2)" />
                      {tech.phone}{tech.extension ? ` ต่อ ${tech.extension}` : ''}
                    </a>
                  )}
                </div>
              );
        ...
        {showModal && (
          <div style={{
            ...
            <div style={{
              ...
              padding: '24px 24px 28px', boxShadow: 'var(--shadow-2)',
            }}>
